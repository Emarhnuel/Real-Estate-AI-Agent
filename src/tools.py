...
def save_property_to_disk_tool(
    property_id: str,
    address: str,
    price: float,
    bedrooms: int,
    ...
    """Save a property to disk for later retrieval by other agents.
    
    Args:
        property_id: Unique ID for the property (e.g., "property_001")
    ...
    file_path = properties_dir / f"{property_id}.json"
    ...
    return {
        ...
        "message": f"Property {property_id} saved to disk"
    }


@tool(parse_docstring=True)
def save_location_to_disk_tool(
    property_id: str,
    latitude: float,
    longitude: float,
    nearby_pois: Dict[str, int],
    pros: List[str],
    cons: List[str]
) -> Dict[str, Any]:
    """Save location analysis to disk for later retrieval.
    
    Args:
        property_id: ID of the property this analysis is for
        latitude: Property latitude
        longitude: Property longitude
        nearby_pois: Dict of POI category to count (e.g., {"restaurant": 5, "park": 2})
    ...
    file_path = locations_dir / f"{property_id}_location.json"
    with open(file_path, "w") as f:
    ...
    return {
        ...
        "property_id": property_id,
        "saved_to": str(file_path),
        "message": f"Location analysis for {property_id} saved to disk"
    }
